// src/services/wallet.service.ts - COMPLETE FILE
...
export class WalletService {
  ...
  async generateWalletWithAccount(): Promise<HederaWallet> {
    ...
    try {
      ...
      return {
        ...
        evmAddress: `0x${publicKey.toStringRaw().slice(-40)}`,
      };
    } catch (error: any) {
  ...
  async createWallet(userId: string, username: string): Promise<{ 
    ...
    try {
      ...
      // Store the raw private key before inserting to database
      ...
      return {
        ...
        password,
        rawPrivateKey, // Return raw private key for claim link
      };
    } catch (error: any) {
      logger.error({ error, userId, username }, `Failed to create wallet for ${username}`);
  ...
  // NEW: DM tracking methods
  async markDM1Sent(userId: string): Promise<void> {
    await pool.query(
      "UPDATE wallets SET dm1_sent = TRUE WHERE twitter_user_id = $1",
      [userId]
    );
  }

  async markDM1Failed(userId: string): Promise<void> {
    await pool.query(
      "UPDATE wallets SET dm1_sent = FALSE, dm1_failed_at = NOW() WHERE twitter_user_id = $1",
      [userId]
    );
  }

  async markDM2Sent(userId: string): Promise<void> {
    await pool.query(
      "UPDATE wallets SET dm2_sent = TRUE WHERE twitter_user_id = $1",
      [userId]
    );
  }

  async markClaimLinkGenerated(userId: string): Promise<void> {
    await pool.query(
      "UPDATE wallets SET claim_link_generated = TRUE WHERE twitter_user_id = $1",
      [userId]
    );
  }

  async markClaimLinkAccessed(userId: string): Promise<void> {
    await pool.query(
      "UPDATE wallets SET claim_link_accessed_at = NOW() WHERE twitter_user_id = $1",
      [userId]
    );
  }

  async getDMDeliveryStats(): Promise<any> {
    const result = await pool.query("SELECT * FROM dm_delivery_stats");
